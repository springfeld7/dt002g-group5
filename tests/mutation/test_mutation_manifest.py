...
from src.transtructiver.mutation.mutation_manifest import (
    MutationManifest,
    ManifestEntry,
)
...
# ===== MutationManifest Positive Cases =====


def test_add_mutation_creates_new_entry():
    """
    Test that adding a mutation creates a new ManifestEntry if none exists.
    """
    manifest = MutationManifest()
    node_id = sample_node_id()
    manifest.add_mutation(node_id, MutationAction.RENAME, {"a": 1}, "RuleX")
...
def test_add_mutation_updates_existing_entry():
    """
    Test that adding a mutation to an existing entry updates it correctly.
    """
    manifest = MutationManifest()
    node_id = sample_node_id()
    manifest.add_mutation(node_id, MutationAction.RENAME, {"foo": 1}, "RuleA")
    manifest.add_mutation(node_id, MutationAction.INSERT, {"bar": 2}, "RuleB")
    entry = manifest.get_entry(node_id)
...
def test_get_entry_returns_none_for_unknown_node():
    """
...
def test_manifest_tracks_multiple_nodes_independently():
    """
    Test that multiple nodes are tracked independently in the manifest.
    """
    manifest = MutationManifest()
    manifest.add_mutation((1, 1), MutationAction.RENAME, {"x": 1}, "R1")
    manifest.add_mutation((2, 2), MutationAction.INSERT, {"y": 2}, "R2")
...
def test_metadata_deep_copy_isolation():
    ...
    manifest.add_mutation(node_id, MutationAction.RENAME, original_meta, "Rule1")
...
def test_metadata_merge_does_not_mutate_previous_history():
    """
    ...
    manifest.add_mutation(node_id, MutationAction.INSERT, {"data": [1]}, "RuleA")
    manifest.add_mutation(node_id, MutationAction.INSERT, {"data": [1, 2]}, "RuleB")
...
def test_action_sequence_integrity():
    """
    ...
    manifest.add_mutation(node_id, MutationAction.RENAME, {"name": "new"}, "Rule1")
    manifest.add_mutation(node_id, MutationAction.DELETE, {}, "Rule2")
...
def test_add_mutation_with_empty_metadata():
    """
    ...
    manifest.add_mutation(node_id, MutationAction.RENAME, {"key": "val"}, "Rule1")
    manifest.add_mutation(node_id, MutationAction.RENAME, {}, "Rule2")
...
def test_multiple_updates_with_same_rule():
    """
    Test that multiple updates with the same rule are recorded separately.
    """
    manifest = MutationManifest()
    node_id = sample_node_id()
    manifest.add_mutation(node_id, MutationAction.RENAME, {"x": 1}, "RuleA")
    manifest.add_mutation(node_id, MutationAction.RENAME, {"y": 2}, "RuleA")
    entry = manifest.get_entry(node_id)
    assert len(entry.history) == 2
...
def test_metadata_merge_with_overlapping_keys():
    """
    Test that overlapping metadata keys are overwritten, not removed.
    """
    manifest = MutationManifest()
    node_id = sample_node_id()
    manifest.add_mutation(node_id, MutationAction.RENAME, {"val": 10}, "RuleA")
    manifest.add_mutation(node_id, MutationAction.RENAME, {"val": 20}, "RuleB")
    entry = manifest.get_entry(node_id)
    assert entry.metadata["val"] == 20  # latest overwrites previous


def test_history_and_metadata_consistency():
    """
    ...
    manifest = MutationManifest()
    node_id = sample_node_id()
    manifest.add_mutation(node_id, MutationAction.RENAME, {"a": 1}, "R1")
    manifest.add_mutation(node_id, MutationAction.INSERT, {"b": 2}, "R2")
    entry = manifest.get_entry(node_id)
